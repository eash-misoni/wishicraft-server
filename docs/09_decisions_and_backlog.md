...
> 2026-09-25 Monitoring Coverage follow-up: NFR-008 / D-111で分離したDLQ 2件のVisible alarmとWeb Errors 2件の既存SNS接続を限定sliceとして実装。4対象はalways-notify。既存queue・policy・application・抑制3件は不変。[契約・手順・別backlog](runbooks/monitoring_coverage.md)。実dev release／通知実証は同runbookの証跡で別途確認し、repository実装だけで完了としない。
...
> 2026-09-18 release review: Game authority fix `c59d94a` は全1,462 testsと両CI成功。live template構造差分はLambda Codeのみだが、実change setが5 State Machine Definition更新を含むため未実行削除。全ASLの意味的比較は同一、signature差の原因は未確定。productionはSTOPPED/HEALTHY・45 OK・受付閉鎖、START未実施。[証跡](evidence/game_package_authority_2026-09-18.json)。
...
## D-099 Discord実行者・主要経過表示（Accepted、限定release Completed）

既存Operationに固定数の到達記録と署名済み実行者名を保存し、一メッセージへ投影する。[契約差分・表示例・適用計画](reviews/discord_progress.md)が正本。D-081/086/087/089と認可/runtime契約は不変。2026-09-12ユーザーGOでAccepted。同日、11 Lambda codeの限定deployと実Discord STATUS一回の保存・表示・配送検証を完了。実証範囲と未実証範囲は正本のcloseout参照。
...
- **状態:** Accepted（2026-09-11ユーザーGO。同日production移行・通常二巡検証Completed）
- **対象:** 既存Gameの通常START/STOPのみ。Phase 9全体や新Game/worldモデルの採用ではない。
- **決定:** Operationへinstance/Game/data/config/runを固定し、hostが排他内で既存Operation/leaseをread-backする。runと実processの識別を分離し、heartbeat/停止予告を新processへ持ち越さない。data directoryは維持する。
...
- **適用結果:** 限定STOP受付例外の追加GO後、canonical STOPで復旧しpreflight run環境修正をhost/CPへ適用。新Operationによる通常START/STOP二巡、既存world保持、exact container削除、新heartbeat identityを実証。最終STOPPED/HEALTHY、両受付UNSET、41 alarm OK、元EBSと5 Snapshot/provenance保持。historical FAILED STARTは不変。[closeout](runbooks/targeted_runtime_migration.md#production-closeout--limited-stop--forward-migration)参照。
...
- **Filesystem:** fixed Host probe v1.4の独立telemetryをSystemState observationへ保存する。既存installed heartbeat probe v1.3は更新不要。正本mount、XFS UUID、sourceのNVMe EBS serial、directory fdのdevice一致と採取前後mount一致を確認して`fstatvfs`する。world/log/secretは読まない。total=`f_blocks*f_frsize`、used=`(f_blocks-f_bfree)*f_frsize`、available=`f_bavail*f_frsize`（bytes）、usage=`100*used/(used+available)`。reserved blockをavailableへ含めない。
- **監視値:** filesystemは10分以内のSystemState/telemetry、fresh same-identity heartbeatとboot照合が揃う場合だけ値を発行する。unknown/missing/source不一致/停止/古い値を0%にしない。80%以上をwarningとする。30 GiB上で概ね20%の余裕を残す運用通知であり、削除・volume拡張・STOPは行わない。復帰は有効値80%未満、または正規停止によるnot-expectedへの移行。unknown時は容量値なしと独立unknown flagを発行する。
- **Alarm:** 新規6 boolean metricsは5分Maximum、2/2、threshold>=1。capacity high以外はmissing=breaching。capacity unknown時にはhigh flagを発行せず、high alarmだけmissing=ignoreにして観測不能から復帰値を捏造しない。unknown/silenceはDataFilesystemObservationUnknownとobserver監視が検出する。CloudWatchのcompleted period評価で2点違反時ALARM、2/2を満たさなくなればOKへ復帰する。gapではCloudWatchのevaluation range/backfill規則により厳密なwall-clock 10分保証ではない。SNSはALARM遷移時のみ、復帰はalarm history/consoleで確認する。高頻度Dimensionを追加せずStage/SystemIdの固定2 dimensionだけを使用する。既存MonitoringObservationUnknownと10分freshness条件は維持し、原因識別用SystemStateObservationStaleを追加する。
- **Cost/retention:** 月額15 USD、50/80/100% actual・100% forecast、8時間running、15分stopped/running、20分not-readyを維持する。dev14日/prod30日をControl Plane全LogGroupでstage設定へ接続する。新規は6 alarm/最大10 custom metrics、1 schedule、追加Lambdaなし。S3 archiveはDeferred、Package S3はPhase 9以降、bootstrap assetsの削除policyは今回変更しない。
- **IAM:** observerはRuntimeHeartbeats一tableのGetItemを追加し、PutMetricDataを既存namespaceに限定する。ReconcileはLocks一tableのGetItemを追加する。SSM/Observed保存は既存の限定権限を再利用し、secret/port/EC2 lifecycle/IAM wildcardの拡張はない。
...
- **Dev validation:** 2026-09-09にRetain/TTL付き`wc-dev-runtime-heartbeats`とTarget roleのtable/canonical key限定GetItem/PutItemをdeployした。初回cycleはAWS CLIのempty GetItem stdoutをproducerがinvalid JSON扱いして安全に失敗し、itemやMinecraft/EC2/SystemState/DNSを変更しなかった。empty stdoutをabsent recordとする直接回帰修正後、READY/player 0のheartbeatを約60秒周期で保存し、instance/runtime/Game/boot、TTL +24h、same-boot zeroの`empty_since`維持、service-only restart continuityを確認した。canonical adapterの古いCAS identityはcondition failureとなりcurrent itemを上書きしなかった。canonical STOP中の最終観測はprotocol unknown/player null/empty nullとなり、停止後はproducer更新が止まった。START後にSystemState observationが10分超となったため既存`MonitoringObservationUnknown`が一時ALARMとなったが、STOPのfresh observation後にOKへ自動復帰した。これはheartbeat stale alarmではない。automatic STOPとheartbeat stale alarmは未実装である。
- **Registered Game correction:** 2026-09-19、A/B固定選択をrunning receiptとhost probeのexact Game/run/instance bindingへ置換したproducerを、正式Game/package authorityから生成したNeoForge-aware stopped migration bundleでproductionへ適用した。Game固有package cache、owner、全artifact hash、Compose environment、停止receipt、old/new producer hashをfile write前に検証し、common runtime/package digestは不変。MATERIALIZED create-survivalの通常START後、同一Game/run/instance/runtime/boot/processで60秒周期3件すべて`player_count=0`（DynamoDB Number）、protocol ready、HEALTHY、45 alarm OKを確認し、通常STOPでcanonical proof・container cleanup・EC2/DNS停止へ収束した。全Game record、既存Operations、volume、9 Snapshot / 16 provenanceは不変で、Discord/Admission/Web受付を復帰した。[runbook closeout](runbooks/phase8_runtime_heartbeat.md#registered-game-heartbeat-production-closeout--2026-09-19)を正本とする。
...
- normal backupはGameごとactive inventoryのnewest 7を保持し、AWS Snapshot StartTimeだけをauthoritative orderingとする。Operation受付時刻`operation_requested_at`、Snapshot create intent/request直前のapplication時刻`wishicraft_created_at`、AWS開始時刻`snapshot_start_time`、検証後のprovenance永続化時刻`provenance_recorded_at`は別概念としてstrict RFC 3339 parse、timezone-aware、UTC正規化可能であることと各sourceとのinstant一致を検証する。異なるclock source間のexact equalityや固定tolerance付き順序をdelete eligibilityへ要求せず、不自然な順序はstructured diagnostic evidenceとする。Snapshot IDをtie breakerにせず、keep/delete境界が同一StartTime instantならcandidate 0とする。
...
- EBS Snapshot Lockを完全paginationで観測し、`governance`、`compliance-cooloff`、`compliance`のactive lockを持つnormal backupはANOMALYとしてrun全体no-deleteにする。lock解除を自動化せず、RETENTION roleへLockSnapshot/UnlockSnapshotを付与しない。初版は`standard` storage tierだけを許可し、archive等はANOMALYとする。
- durable Backup provenanceは非TTL Backups tableのSnapshot ID recordとOperation ID uniqueness recordをcreate-only transactionで保持する。新規BACKUPはSnapshot検証後にprovenance、Operation SUCCEEDED、Lock/Current Operation cleanupを原子的に確定し、結果不明時はexact read-backだけで収束する。既存Phase 8B/8C recordはproduction read-only evidenceを再検証した別gateのconditional backfillまでANOMALYを維持する。
...
- **Dry-run workflow dev evidence:** 2026-09-08にreal `/mc backup`から3件目のnormal Snapshotと3組目のprovenance pairが通常terminalizationで自動作成されることを検証後、shared Admission/global Lockを使うdry-run-only Standard workflowをdeployした。初回RETENTIONはlist型resultを既存DynamoDB serializerが拒否して明示FAILEDとなったが、Snapshot mutationなしでLock/Current Operationを解放した。scalar evidenceへ限定する回帰修正後、Operation `op-c4bc275b-0622-4c8f-a6e7-c04eb948c017`はinventory 4、KEEP 3、CANDIDATE 0、EXCLUDED 1、ANOMALY 0、planned/delete action 0、Recycle Bin rule 0でSUCCEEDEDした。normal backupは全件standard、active Snapshot Lock 0である。State Machineにdelete path、roleにDeleteSnapshot permissionはなく、CloudTrailのDeleteSnapshot/tag/lock mutationも0、関連alarmは修正後OKへ回復した。実Delete releaseはnatural 8件到達後の別gateのままである。
...
- **日付:** 2026-09-02
- Phase 8 MVPはroot EBSを除外し、current persistent encrypted Data EBSだけをbackupする。BACKUPはD-026/D-074のshared Admissionとglobal Lockを使用し、START/STOP/BACKUP同士を排他する。fresh Desired STOPPED、Actual/Observed stopped、HEALTHY、discrepancyなし、Lock/current operationが当該BACKUP所有である場合だけ進め、BACKUP自身はSTOP/STARTしない。D-090 `WishicraftCreatedAt`はOperation受付時刻ではなく、WishicraftがCreateSnapshot intent/request直前に生成するapplication timestampである。
...
2026-09-23 user-requested slice。[正式契約](reviews/game_restore.md)、[checkpointとrollback](runbooks/game_restore.md)。shared-volume v2 provenanceをauthorityに、operator管理の一時read-only EBSから同一Game/packageだけを新generationへ抽出する。現在world／他Game／Control Plane metadataは巻き戻さない。D-111 maintenance-admin経路と既存STARTを組み合わせ、application IAM／巨大state machineを追加しない。[2026-09-24限定実証](evidence/game_restore_resumed_2026-09-24.md)で保持済みPREPAREDからcommit・復元先START/内容/STOP・正式rollback・元world START/内容/STOPを完遂し、両world保持・元world選択・STOPPED/HEALTHY・49 alarms OKへ戻した。Paper/NeoForge実RESTORE・全失敗経路・災害復旧は未実証。
