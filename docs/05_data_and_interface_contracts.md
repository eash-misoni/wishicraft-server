...
- STATUS/START/STOPは同じshared Admissionとkey contractを使用する。Interaction tokenは上記短期ACK lifecycleだけでprocess memoryに保持し、保存・log出力しない。progress/finalはInteraction tokenではなくOperation単位のD-086通常Bot messageへ投影する。
