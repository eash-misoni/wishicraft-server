...
> D-109 NULL修正契約: 完全Gameの読込みはGame/Operation整数型decoderを再利用し、NULLをNoneへ復元。不正な複数type keyは拒否。START/STOPのfailはGame再読込みを必須にせず、既存exact lease/Lock/Current所有条件でFAILEDと解放をatomicに確定する。成功・runtime副作用のbinding検証は維持。[根拠と検証](runbooks/neoforge_production.md#forward-fix-full-game-decoding-and-owned-failure-closure)。
...
RETENTION operator entrypointはshared Admission Lambdaへ`operation_type=RETENTION`、`requested_by=ADMIN`、canonical Gameとcallerが一度固定した`retention:` namespaceのidempotency keyを送る。State Machineを直接開始しない。dry-run resultはinventory/KEEP/CANDIDATE/EXCLUDED/ANOMALY/deletion-plan/delete-actionの各countとRecycle Bin rule countを保持し、production dry-run-only releaseの`delete_action_count`は必ず0とする。個別Snapshot identityはAWS inventory、provenance、execution logを相互参照する内部運用証跡とし、Operation resultの汎用DynamoDB serializerが未対応のlist型を保存しない。
...
2026-09-22 completion CAS correction: after a successful Discord API call, a conditional completion conflict is a no-op only after consistent read-back proves the same Operation/type/channel, deterministic delivery ID and message ID, with authoritative progress strictly newer than the attempted revision. Before a newer consumer claims, delivery must still be PENDING at the attempted revision under the same attempt, with unchanged delivered revision/outcome. After a newer claim, its nonempty distinct attempt and coherent delivery revision/receipt must lie between attempted and current progress. The stale invocation performs no write and does not deliver the newer revision. Same-revision conflicts (including already-delivered), revision regression, identity/owner/state mismatch and genuine API failure completion conflicts remain errors. This supersedes the historical same-revision terminal no-op exception; ordinary pre-delivery idempotency and Stream/SQS retry semantics are unchanged. [Ordering, proof and release boundary](runbooks/discord_progress_supersede.md).
...
- Phase 7G-3では署名・parse・認可後、Discord callback endpointへtype 5 ephemeral Deferred Channel MessageをAdmission前に送る。callback成功時だけ既存Admission Lambdaへ`RequestResponse`で渡し、idempotency keyを`discord:<interaction_id>`とする。ACKはOperation受付を意味せず、Admission success/failure後にoriginal ephemeral responseをsafe acknowledgementへeditし、元HTTP requestは202/empty bodyで終了する。ACK失敗はAdmission 0へfail closedし、inline callbackとの二重応答をしない。
- STATUS/START/STOP/BACKUPは同じshared Admissionと`discord:<interaction_id>` key contractを使用する。Interaction tokenは上記短期ACK lifecycleだけでprocess memoryに保持し、保存・log出力しない。progress/finalはInteraction tokenではなくOperation単位のD-086通常Bot messageへ投影する。BACKUPの公開文は受付、作成・検証中、完了、未完了だけを示し、Snapshot/Volume/Account IDやraw AWS errorを含めない。
...
- Command ingressはstage固定Guild、operation channel、member rolesを検証する。STATUS/START/STOPはplayer roleまたはadmin role、BACKUPはadmin roleだけを許可し、admin channelは許可しない。
- START/STOP/BACKUPは既存のlock付きAdmissionを呼び、STATUSはLock/Current Operationなしの既存STATUS admissionを呼ぶ。Command LambdaはBACKUP task、State Machine、Snapshot APIを直接呼ばない。
