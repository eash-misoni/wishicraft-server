...
| 4 | **Whitelist Management — production Completed** | [D-106契約](reviews/whitelist_management.md)と[移行gate](runbooks/whitelist_management.md)。Common ∪ Game-specific追加許可、Web authoritative、Admin限定短期Admission、次回START/SWITCH/RESET起動時のprojection。初期Common空、A/B-specificへ既存access完全一致で移行済み。D-105 positive E2E deferredを維持し、OP/chat/runtime一般化は含めない。 |
