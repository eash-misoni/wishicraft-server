...
> D-106 Whitelist Management: repository実装・production release承認済み。[現在のrelease contract](reviews/whitelist_management.md)と[移行gate](runbooks/whitelist_management.md)。D-105のpositive CREATE/materialization deferredは維持。
...
D-103 Web URL StabilizationはAccepted・production Completed（2026-09-13）。canonical originは`https://web.wishicraft.net`。DNS/TLSと新OAuth/session E2E、旧redirect削除後reloginを確認した。[移行・証跡runbook](runbooks/web_custom_domain.md)を正本とする。D-101の順序は維持し、Existing Operations via WebはD-104 Accepted・production Completed。Minimal Game CreationはD-105 Accepted / production deployed, positive CREATE/materialization deferred。最初の実用Gameによるpositive E2Eまでfully production Completedとはしない。
...
| 4 | **Whitelist Management — production release承認済み・実行前** | [D-106契約](reviews/whitelist_management.md)と[移行gate](runbooks/whitelist_management.md)。Common ∪ Game-specific追加許可、Web authoritative、Admin限定短期Admission、次回START/SWITCH/RESET起動時のprojection。実A/B移行は完全一致確認後。D-105 positive E2E deferredを維持し、OP/chat/runtime一般化は含めない。 |
...
4. **Completed（2026-09-10）:** D-093によりGame設定の30分idle、5分warning、1分Evaluator、durable AutoStopIntent、deterministic STOP/SCHEDULE admissionをdevへreleaseした。
5. warningは同一empty periodに一件だけ配送し、実delivery成功から最低5分を確保する。再接続、unknown、stale、boot/empty period変更、terminal failureはfail closedし、同periodでwarning/STOPを再生成しないことをproductionで確認した。
6. existing STOP workflowのDesired変更前に、Lock所有下のfresh Reconcileとfixed Host Runtime direct player probeを追加した。初回`GAME_ID`不足はpre-commit FAILED、次periodのDynamoDB Decimal decode不整合はpre-commit CANCELLEDとなり、いずれもDesired/Minecraft/EC2/DNSを変更しなかった。既存stale recoveryとtyped decoderを回帰test付きで修正した。
7. canonical 30分/5分を短縮しない新empty periodで、warning一件、STOP/SCHEDULE Admission一件、final gate、既存graceful STOP、EC2 stopped、DNS absent、Operation SUCCEEDED、Lock/Current Operation解放、final HEALTHYまでproduction E2Eを完了した。historical FAILED/CANCELLED Operationは保持している。
