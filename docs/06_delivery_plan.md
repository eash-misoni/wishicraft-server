...
D-099 release/証跡整理Completedを維持。次の委任単位は[静的Web準備](reviews/user_guide_web.md)。Markdown再利用、schema/runtime投影、local build/preview、実ブラウザとCI検証、公開計画を一つの完了単位とする。準備完了HEAD `1bab86b`から13ページ化・実用的な外観へ修正し、Game中心の参加導線と公開Game登録からの生成へ整理し、ローカルレビューへ戻す。hosting・閲覧者・認証・URLの確定と実公開は別承認。Phase 9全体やPhase 13管理Webの完了を意味しない。
