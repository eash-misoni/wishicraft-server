...
# v2 writes this root-only context under its host lock, after Operation/lease validation.
if [[ -e "$RUNTIME_DIR/runtime-run.env" ]]; then
  [[ -f "$RUNTIME_DIR/runtime-run.env" && ! -L "$RUNTIME_DIR/runtime-run.env" ]] || fail
