"""Offline, exact stopped-host catalog append; immutable Game records are inputs only."""
...
BASELINE = "866f6ca"
BUNDLE_PATH = "/var/tmp/wishicraft-catalog-terralith-v1"
...
def prepare(
    ...
    *,
    ...
    edge_path = root / "src/wishicraft/artifacts/catalog-transition.json"
    ...
    changes.append(
        (
            "/usr/local/libexec/wishicraft/catalog-transition.json",
    ...
    plan = {
        ...
        "backup_namespace": "catalog-terralith-v1",
    ...
    installer = installer.replace("/var/tmp/wishicraft-targeted-runtime-v1", BUNDLE_PATH).replace(
        'namespace != "two-game-v1"', 'namespace != "catalog-terralith-v1"'
    )
